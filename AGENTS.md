...
- **Key entry points.** The launcher at `public/index.html` links to the four feature modules (`canvas.html`, `grid.html`, `draw.html`, `timeline.html`) and some secondary pages like `public/tree.html`. All of them load `public/js/prompt.js` plus the appropriate module scripts and expect the `?v=2025.12.23` query string on module links to bust caches when static assets change.
...
- **Cache/version reminders.** All launcher links and in-page navigation append `?v=2025.12.23` to their module URLs and `js/prompt.js`, so updating static assets means bumping that query string across templates, navigation, and the hero/version badge if you display it. Keep the `2025.12.23` value consistent until you change the static bundle.
