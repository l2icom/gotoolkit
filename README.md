...
- **Go-Think** (`public/index.html`) : grille de slides éditables avec IA pour générer contenus, suggestions par section et export PPTX/PNG/JSON.
- **Go-Draw** (`public/draw.html`) : générateur de diagrammes Mermaid/Excalidraw avec titre détecté automatiquement et partage optionnel.
- **Go-Plan** (`public/plan.html`) : timeline interactive (Vis.js) avec IA pour créer/modifier un planning, export texte/image/Excel/JSON.
...
## Pour quoi faire ?
- **Préparer un atelier** : esquisser une storyline (Think), dessiner un parcours (Draw), poser les jalons (Plan) sans ouvrir de suite bureautique.
...
- **Exports natifs** : Think → PPTX/PNG/JSON, Draw → PNG/JSON Mermaid, Plan → texte/image/Excel/JSON ; rien ne sort sans action explicite.
...
- **Go-Discovery** : canevas OST / personas / insights reliés à Think.
- **Go-Metrics** : cadrage North Star et suivis trimestriels, connecté aux timelines Plan.
...
2. Ouvrir une app : `index.html` (Think), `draw.html` (Draw), `plan.html` (Plan).
